...
\begin{block}{}\vspace*{-.2cm}
\begin{align*}
  \text{CRPS}(F,y) &= \int_{-\infty}^\infty (F(x) -1\{y \le x\})^2 dx \\
  &= \text{E}_F|Y-y| - {\textstyle\frac12} \text{E}_F|Y-Y'|
\end{align*}
\end{block}

 * $y$ is observation, $F$ is estimated forecast distribution
 * $Y$ and $Y'$ are iid with cdf $F$
...
fcast <- fc %>%
  filter(.model=='ets') %>%
  right_join(best) %>%
  as_fable(resp=rlang::exprs(Trips), dist=.distribution)
